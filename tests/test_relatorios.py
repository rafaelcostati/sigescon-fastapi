...
async def setup_for_reports(async_client: AsyncClient, admin_headers: Dict, admin_user: Dict) -> Dict:
    ...
    # 2. Criar um Contrato
    contratado_resp = await async_client.post("/contratados/", json={"nome": f"Empresa Relatorio {uuid.uuid4().hex[:6]}", "email": f"empresa.relatorio.{uuid.uuid4().hex[:6]}@teste.com"}, headers=admin_headers)
    ...
    pendencia_resp = await async_client.post(f"/contratos/{contrato['id']}/pendencias/", json=pendencia_data, headers=admin_headers)
...
async def test_full_report_workflow(async_client: AsyncClient, admin_headers: Dict, setup_for_reports: Dict):
    ...
    submit_resp = await async_client.post(
        f"/contratos/{contrato_id}/relatorios/",
        data=report_data,
        files=files,
    ...
    status_aprovado_resp = await async_client.get("/statusrelatorio/", headers=admin_headers)
    ...
    analise_resp = await async_client.patch(
        f"/contratos/{contrato_id}/relatorios/{relatorio_id}/analise",
        json=analise_data,
