...
# --- CORREÇÃO: Removido scope="module" de todas as fixtures ---
...
@pytest.mark.asyncio
async def test_crud_contrato_workflow(async_client: AsyncClient, admin_headers: Dict, contract_prerequisites: Dict):
    """Testa o fluxo completo de CRUD para Contratos."""
    
    # 1. CREATE
    contrato_data = {
        "nr_contrato": f"TESTE-{uuid.uuid4().hex[:8]}",
    ...
    create_response = await async_client.post("/contratos/", json=contrato_data, headers=admin_headers)
    assert create_response.status_code == 201
    created_contrato = create_response.json()
    assert created_contrato["nr_contrato"] == contrato_data["nr_contrato"]
    ...
    get_all_response = await async_client.get(f"/contratos/?nr_contrato={contrato_data['nr_contrato']}", headers=admin_headers)
    ...
    # 4. UPDATE
    update_data = {"objeto": "Este é um objeto de contrato atualizado."}
