...
import uuid
import random # <-- Importação adicionada
...
async def test_crud_modalidades_workflow(async_client: AsyncClient, admin_headers: Dict):
    ...
    # 2. READ ALL
    print("\n--- Testando READ ALL ---")
    get_all_response = await async_client.get("/modalidades/", headers=admin_headers)
    ...
    # 4. DELETE
    print("\n--- Testando DELETE ---")
    delete_response = await async_client.delete(f"/modalidades/{modalidade_id}", headers=admin_headers)
    assert delete_response.status_code == 204
    print(f"✓ Modalidade ID {modalidade_id} deletada com sucesso.")

    # 5. VERIFY DELETE
    print("\n--- Verificando DELETE ---")
    verify_response = await async_client.get("/modalidades/", headers=admin_headers)
...
async def test_non_admin_cannot_write(async_client: AsyncClient, admin_headers: Dict):
    ...
    create_fail = await async_client.post("/modalidades/", json={"nome": "Tentativa Fiscal"}, headers=fiscal_headers)
    assert create_fail.status_code == 403
    
    # Fiscal TENTA atualizar (deve falhar)
    update_fail = await async_client.patch("/modalidades/1", json={"nome": "Tentativa Fiscal"}, headers=fiscal_headers)
    assert update_fail.status_code == 403

    # Fiscal TENTA deletar (deve falhar)
    delete_fail = await async_client.delete("/modalidades/1", headers=fiscal_headers)
